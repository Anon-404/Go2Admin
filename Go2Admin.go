...
func banner() {
    banner := `
   ______     ___   ___       __          _     
  / ____/___ |__ \ /   | ____/ /___ ___  (_)___ 
 / / __/ __ \__/ // /| |/ __  / __ '__ \/ / __ \
/ /_/ / /_/ / __// ___ / /_/ / / / / / / / / / /
\____/\____/____/_/  |_\__,_/_/ /_/ /_/_/_/ /_/ 
 `
...
func version() {
	boldYellow.Println("version 1.0\n")
	return
}

func help() {
    boldCyan.Println("╔═══════════════════════════════════════════════════════════╗")
	boldCyan.Println("║                        Go2Admin                           ║")
	boldCyan.Println("║             Web Admin Panel Brute-Forcing Tool            ║")
	boldCyan.Println("╚═══════════════════════════════════════════════════════════╝")
    boldCyan.Println()

    boldCyan.Println("Usage:")
    boldWhite.Println("  ./Go2Admin -d <domain>")
    boldCyan.Println()

    boldCyan.Println("Options:")
    boldGreen.Print("  -h ")
    boldCyan.Print(" --help")
    boldWhite.Println("         : Display this help menu")

    boldGreen.Print("  -v ")
    boldCyan.Print(" --version     ")
    boldWhite.Println(" : Show the tool's version")

    boldGreen.Print("  -d ")
    boldCyan.Print(" --domain      ")
    boldWhite.Println(" : Specify the target domain")
    fmt.Println()

    boldCyan.Println("Examples:")
    boldWhite.Println("  ./Go2Admin -d example.com")
    boldWhite.Print("  ./Go2Admin -d example.com wordlist.txt")
    boldYellow.Println("(Custom wordlist)")
    boldCyan.Println()

    boldCyan.Println("╔═══════════════════════════════════════════════════════════╗")
    boldCyan.Println("║             Thank you for using Go2Admin!                 ║")
    boldCyan.Println("╚═══════════════════════════════════════════════════════════╝")
}
...
func dbgbrute(domain string, word string) {
    ...
    if conn.StatusCode >= 100 && conn.StatusCode <= 199 {
        boldYellow.Printf("\033[1m\033[33m%-40s status : %d\n", dom, conn.StatusCode)
		i = append(i, dom)
        // 200-299
    } else if conn.StatusCode >= 200 && conn.StatusCode <= 299 {
    	boldGreen.Printf("\033[1m\033[32m%-40s status : %d\n", dom, conn.StatusCode)
		i = append(i, dom)
        // 300-399
    } else if conn.StatusCode >= 300 && conn.StatusCode <= 399 {
    	boldBlue.Printf("\033[1m\033[34m%-40s status : %d\n", dom, conn.StatusCode)

		i = append(i, dom)
        // 400-499
    } else if conn.StatusCode >= 400 && conn.StatusCode <= 499 {
        boldRed.Printf("\033[1m\033[31m%-40s status : %d\n", dom, conn.StatusCode)
...
func main() {
    ...
    if len(os.Args) < 2 {
        boldBlue.Println("\nUsage: ./Go2Admin -d [domain]")
        boldBlue.Println("Manual: ./Go2Admin -h")
        boldBlue.Println("Version: ./Go2Admin -v")
        return
    }
    ...
    if len(os.Args) < 3 {
        ...
        boldBlue.Println("Usage: ./Go2Admin -d <domain>")
        boldBlue.Println("Manual: ./Go2Admin -h")
        boldBlue.Println("Version: ./Go2Admin -v")
        return
    }
	...
	filename := "default.txt" // Default filename

    if len(os.Args) >= 4 {
        filename = os.Args[3] // Use the provided wordlist filename
    }
